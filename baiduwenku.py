...
headers = {
    'user-agent': random.choice(fake_ua),
}


def getSize(filepath, preSize):
    size = os.stat(filepath).st_size
    new_size = size + preSize + 1
    return new_size
...
def getPic(url, name):
    resp = requests.get(
        headers=headers,
        url=url)
    with open(f'./pic/{name}.jpg', 'wb') as fp:
...
if __name__ == '__main__':
    ...
    if not os.path.exists('./pic'):
        os.mkdir('./pic')
    while True:
        url = f'https://wkretype.bdimg.com/retype/zoom/efed1e3caef8941ea66e05b5?pn=1&o=jpg_6&md5sum=b79b9539a208baa61cd059e92205c9f1&sign=00f787d69c&png=0-13142&jpg={size}-'
        getPic(url, name)
        size = getSize(f'./pic/{name}.jpg', size)
        if os.stat(f'./pic/{name}.jpg').st_size <= 200:
            break
        name += 1
